...
export async function requestPasswordReset({ email }) {
  try {
    ...
    const message = extractMessage(error, "Nao foi possivel iniciar a recuperacao de senha.");
    const err = new Error(message);
    err.cause = error;
    throw err;
  }
...
export async function resetPassword({ token, password }) {
  try {
    ...
    const message = extractMessage(error, "Nao foi possivel redefinir a senha.");
    const err = new Error(message);
    err.cause = error;
    throw err;
  }
